...
h2("■職務要約")
body([
    "プロジェクトマネジメント歴11年。要件定義から現場実装まで、経営層・現場・バックオフィス間の認識差を構造化し、DX推進・業務自動化・組織基盤づくりを一気通貫でリードすることを一貫した強みとしています。",
    "大手自動車メーカーや通信キャリア企業での要件定義～基本設計、後進育成やチーム生産性向上、国内外での折衝・資料作成の経験を土台に、現在は総合建設業のDX・AI推進顧問として、ANDPAD、KING OF TIME、LINE WORKSなど複数SaaSを横断した業務自動化、就業規則改定、Claude Codeを活用した意思決定プロセスのドキュメント化までを実践し、業種を超えて成果を出しています。",
    "近年「Forward Deployed Engineer（FDE）」と呼ばれる、顧客の現場に入り込み要件定義から導入定着・運用まで一気通貫で担う働き方が国内外で注目されていますが、上記の実践はこのスタイルと重なるものです。",
])

h2("■活かせる知識・経験")
body([
    "・組織DX推進: 建設業にて経営層・現場・バックオフィス間の認識差を構造化し、業務自動化/就業規則整備/インフラ改善を一気通貫で推進。",
    "・SaaS横断の業務自動化: ANDPAD、KING OF TIME、LINE WORKS等を連携し、紙・Excel・チャットに分断された業務フローをPython/GASで再設計。",
    ...
    "・技術的知識: Python、AWS、GAS、Figma、Copilot、M365、マーケティング戦略。",
])
...
entries.append({
    "company": "■個人事業主（AI/DX推進アドバイザー、複数社伴走支援）",
    "biz": "事業内容：中小企業複数社に対するAI活用・DX推進のアドバイザリー",
    "period": "2026年8月\n～現在",
    "summary": [
        "概要：業務委託・スポット契約で複数のクライアント企業に対しAI・DX推進を伴走支援",
        "役割：経営層と現場をつなぐPM・DX推進アドバイザー",
        "規模：2社（拡大中）",
    ],
    "body": [
        "・森信建設株式会社：2025年10月に業務委託でDX・AI推進顧問として参画、2026年4月に正社員化、2026年8月に業務委託へ再移行し継続支援（詳細は下記エントリ参照）",
        "・有限会社シンワ技術：2026年〜、Codex/Claude Code活用による業務効率化のスポットAIアドバイザー契約を締結し、社内システムの内製化を支援",
    ],
    "knowledge": "Claude Code、Codex、生成AI活用推進、業務委託契約マネジメント",
})
...
entries.append({
    ...
    "summary": [
        ...
        "契約形態：2025年10月業務委託→2026年4月正社員→2026年8月業務委託（現在）",
