...
h2("■職務要約")
body([
    "総合建設業にてDX・AI推進顧問として、経営層と現場双方の認識差を構造化し、業務自動化・就業規則改定・ネットワーク整備までを一気通貫で推進。ANDPAD、KING OF TIME、LINE WORKSなど複数SaaSを横断した業務再設計や、Claude Codeを活用した意思決定プロセスのドキュメント化にも取り組み、属人化しない業務基盤づくりに貢献しています。",
    "プロジェクトマネジメント歴11年。大手自動車メーカーや通信キャリア企業での要件定義～基本設計、後進育成やチーム生産性向上、国内外での折衝・資料作成にも実績があり、この基盤を活かして現在は業種を超えたDX推進に取り組んでいます。",
])
